...
func TestValidTokenSucceeds(t *testing.T) {
	...
	token := createUser(t, issuer, "audience", "radix1")
...
func TestMultipleIssuers(t *testing.T) {
	...
	issuer3 := createServer(t)
	mux := http.NewServeMux()

	token := createUser(t, issuer, "audience", "radix1")
	token2 := createUser(t, issuer2, "audience", "radix2")
	token3 := createUser(t, issuer3, "audience", "radix3")
...
func TestInvalidAudienceFails(t *testing.T) {
	issuer := createServer(t)
	token := createUser(t, issuer, "audience-invalid", "radix1")
...
func TestAuthInvalidSubjectFails(t *testing.T) {
	...
	token := createUser(t, issuer, "audience", "radix3")
	require.NoError(t, err)
...
func createUser(t *testing.T, issuer, audience, subject string) string {
